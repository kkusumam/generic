...
features = ['mean_x', 'std_x', 'mean_y', 'std_y', 'mean_vel', 'std_vel',
            ...
            'std_dist', 'mean_freq', 'std_freq', 'amplitude']
...
for i in range(data.shape[0] - window_size):
    ...
    dt = 1 / 30  # Assuming a frame rate of 30 fps
    ...
    # Compute frequency using distances between adjacent points
    freq = np.fft.fft(dists)
    ...
    # Compute amplitude as the maximum distance between any two hand points
    amplitude = np.max(dists)
    X[i, 14] = amplitude
...
for i in range(data.shape[0] - window_size):
    ...
    angles = np.arctan2(diff[:,1], diff[:,0])
    angles[angles < 0] += 2*np.pi
    dists = np.sqrt(np.sum(diff**2, axis=1)) * 30
    X[i,8] = np.mean(angles)
    X[i,9] = np.std(angles)
    X[i,10] = np.mean(dists)
    X[i,11] = np.std(dists)
    ...
    # Assign the label based on the severity of the tremor in the window
    if np.mean(vel) > 300 and np.mean(acc) > 1000:
        y[i] = 0  # severe
    elif np.mean(vel) > 150 and np.mean(acc) > 500:
        y[i] = 1  # moderate
    else:
        y[i] = 2  # mild
